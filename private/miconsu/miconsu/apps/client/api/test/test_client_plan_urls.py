...
from client.api.views import ClientPlanViewSet
...
class URLTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.list_view = ClientPlanViewSet.as_view({'get': 'list'})
        self.create_view = ClientPlanViewSet.as_view({'post': 'create'})
        self.detail_view = ClientPlanViewSet.as_view({'get': 'retrieve'})
        self.update_view = ClientPlanViewSet.as_view({'put': 'update'})
        self.delete_view = ClientPlanViewSet.as_view({'delete': 'destroy'})
    ...
    def test_create__200__app_administrator(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, {'id': 5, 'name': 'Nuevo Plan de servicios'})


    def test_detail__403__not_authenticated(self):
        """Detail View should not be available for Anonymous user"""
        response = self.detail_view(self.get_request, ClientPlan.objects.last().id)
        self.assertEqual(response.status_code, 403)
    ...
    def test_update__object_is_updated(self):
        """Object is updated"""
        client_plan_id = 2
        client_plan = ClientPlan.objects.get(id=client_plan_id)
        original_name = client_plan.name
        data = {
            'name': 'Plan Modificado',
        }
        put_request = self.factory.put("", data, format="json")
        force_authenticate(put_request, user=self.app_admin)
        response = self.update_view(put_request, pk=client_plan_id)
    ...
    def test_delete__object_is_deleted(self):
        """Object is deleted"""
        object_id = 3
        instance = ClientPlan.objects.get(id=object_id)

        force_authenticate(self.delete_request, user=self.app_admin)
        response = self.delete_view(self.delete_request, pk=object_id)

        filtered_objects = ClientPlan.objects.filter(id=object_id)
